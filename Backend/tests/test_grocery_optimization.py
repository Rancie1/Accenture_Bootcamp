...
@patch('services.grocery_service.call_n8n_webhook', new_callable=AsyncMock)
def test_grocery_optimization_endpoint_success(mock_n8n, client, seed_demo_data):
    """Test successful grocery optimization with price predictions"""
    # Use the seeded database session
    db_session = seed_demo_data
...
@patch('services.grocery_service.call_n8n_webhook', new_callable=AsyncMock)
def test_grocery_optimization_with_price_predictions(mock_n8n, client, seed_demo_data):
