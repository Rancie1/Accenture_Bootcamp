...
const Results = () => {
  ...
  const initialTransportCost =
    parsedTransportCost !== null ? parsedTransportCost : transport.defaultCost;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 pb-24">
        ...
        {itemsWithoutPrice.length > 0 && itemsWithPrice.length > 0 && (
          <p className="text-xs text-amber-600 dark:text-amber-400 mt-2">
            * {itemsWithoutPrice.length}{" "}
            {itemsWithoutPrice.length === 1 ? "item has" : "items have"} no
            price — total may be higher.
          </p>
        )}
      </div>
      ...
      {/* ── Transport Card ──────────────────────────────────────────── */}
      <div
        className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5 mb-4 border-l-4 ${transport.borderClass}`}
      >
            ...
            {showTransportEdit ? (
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && setShowTransportEdit(false)
                  }
                />
              </div>
            ) : (
      ...
      {budget > 0 && (
        <div
          className={`rounded-2xl shadow-lg p-5 mb-4 border-2 ${
            isUnderBudget
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <DollarSign
                size={20}
                className={isUnderBudget ? "text-green-600" : "text-red-600"}
              />
              <h3 className="font-bold text-gray-900 dark:text-white">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {isUnderBudget ? "Saved" : "Over by"}
              </p>
              <p
                className={`text-lg font-bold ${isUnderBudget ? "text-green-600" : "text-red-600"}`}
              >
          ...
          <div className="mt-3">
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div
                ...
                style={{
                  width: `${Math.min(100, (finalCost / budget) * 100)}%`
                }}
              />
            </div>
          </div>
